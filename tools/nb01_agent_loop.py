...
cells = [
    ...
    md(
        ...
        "2. **Step 2** — the model reads `15`, realizes \"that's addition, not multiplication, so the "
        "tool didn't give me what I want\" — and **answers from its own training data**: 7 × 8 = 56\n"
        "\n"
        "**The model bypassed the tool.** It didn't crash. It didn't loop with `add` to brute-force "
        "the multiplication. It just used internal knowledge.\n"
        "\n"
        "This is the **cognitive-shortcut** behavior, and it's a real thing about how modern agents "
        "work that 2024-era tutorials don't cover:\n"
        "\n"
        ...
        "\n"
        "**Senior interview answer:** *\"You can't prove tool-use behavior by reading the prompt — you "
        "must observe the trace. If you want forced tool use, you need `tool_choice=\"required\"`, or "
        "schema design that makes the tool's role explicit, or remove the model's escape hatch by "
        "asking something genuinely outside training.\"*\n"
        "\n"
        "**The implication for engineers**: don't assume your agent will use your tools just because "
        "you registered them. Run the trace. Always."
    ),
    md(
