...
cells = [
    ...
    md(
        ...
        "\n"
        "We'll register a tool that returns vague \"hmm not sure, search again\" results. The LLM keeps "
        "trying. Without the budget cap it would burn through quite a bit; we cap it at $0.001."
    ),
    code(
        ...
        "    \"Find me a definitive answer about the population of Atlantis in 2026.\",\n"
        "    registry,\n"
        "    \"You are a research assistant. Use web_search until you have a definitive answer.\",\n"
        "    max_steps=15,\n"
        "    budget_usd=0.001,    # very tight — should trip\n"
        ")\n"
    ...
    md(
        "Note that even though the budget tripped, **the user got a real answer** (probably along the lines "
        "of *\"I couldn't find definitive information…\"*). That's the whole point of graceful termination — "
        "you serve the user, you don't 500 them.\n"
    ),
    md(
        "## 6. Trip the loop guard — semantically-equivalent retries\n"
        "\n"
        "Here we'll feed a query that almost always triggers the LLM to call the same tool twice with "
        "identical args (or close to it). The guard catches the literal repeat after 3 fires."
    ),
    code(
        "result = await run_guarded(\n"
        "    \"Search for 'webgpu adoption 2026'. If unclear, try again with the same query.\",\n"
        "    registry,\n"
        "    \"You are a research assistant. Follow the user's instructions exactly.\",\n"
        "    max_steps=10,\n"
        "    budget_usd=0.05,\n"
        "    repeat_threshold=3,\n"
        ")\n"
