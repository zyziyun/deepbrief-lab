"""Generator for notebooks/03_termination.ipynb."""
...
cells = [
    md(
        "# 03 — Termination & Cost\n"
        "\n"
        "> *\"The same request used to close in 3-4 steps. After a model upgrade it spins for 20+ steps "
        "and ends with timeout. In 15 minutes the agent makes 60+ steps and spends ~$12 on a task that "
        "usually costs $0.08.\"*\n"
        "\n"
        "That's a **150× cost overrun**, and the system *didn't fail* — it just slowly burned tokens. "
        "This is the failure mode that costs companies real money in 2026, and it's where mid-level "
        "engineers fall short of senior.\n"
        "\n"
    ...
    md(
        ...
        "\n"
        "Defense is **stacked**: never rely on one termination condition. Production agents have at "
        "least four hard caps (steps, tokens, wall-clock, cost) plus soft signals (loop fingerprint, "
        "no-progress detection).\n"
    ),
    md(
        "## 2. CostMeter — bound dollars per run\n"
        "\n"
        "Lives in `src/deepbrief/agents/guards.py`. Pricing is in a module-level dict — keep it in config "
        "and refresh on price changes."
    ),
    code(
    ...
    md(
        ...
        "\n"
        "Hash `(tool_name, args)` on every call. If the same fingerprint appears N times, the agent is "
        "stuck — return a canned message to the LLM as a tool result. The LLM almost always rephrases "
        "or gives up gracefully.\n"
        "\n"
        "Doesn't catch *semantic* drift (`\"weather Paris\"` vs `\"current Paris weather\"`). For that "
        "you'd cosine-similarity-embed the args. We won't need that for first-iteration agents."
    ),
    code(
    ...
    md(
        ...
        "\n"
        "We'll register a tool that returns vague \"hmm not sure, search again\" results. To force the "
        "agent to actually loop (modern `gpt-4o-mini` tries to give up gracefully on truly unanswerable "
        "questions), we tell the system prompt to search **at least 5 times**. Combined with a tight "
        "$0.0001 budget, the cap will fire mid-loop."
    ),
    code(
    ...
    md(
        ...
        "- A non-empty `answer` — the **graceful termination** block ran after the trip, forcing the LLM "
        "to produce a final text response with `tool_choice=\"none\"`\n"
        "\n"
        "**This is the whole point of graceful termination** — the user gets a real answer ("
        "*\"I couldn't find definitive information…\"*), ops gets a budget-trip event in the trace, you "
        "don't return a 500 to the user. Three good outcomes from one failure mode.\n"
        "\n"
        "If your run shows `terminated_by: natural` instead — `gpt-4o-mini` decided to stop on its own "
        "before the budget tripped. Re-run, or lower `budget_usd` further (e.g., `0.00003`). Model "
        "behavior is non-deterministic (see notebook 01 §3)."
    ),
    md(
        "## 6. Trip the loop guard — identical-arg retries\n"
        "\n"
        "To reliably fire the guard, we force the model to issue identical args via the prompt and lower "
        "the threshold to **2**. (With `threshold=3` and modern `gpt-4o-mini`, the model usually gives "
        "up after 2 unhelpful searches on its own — never giving the guard a chance.) The pattern below "
        "is the actual demo shape; in production you'd typically use threshold 3 or 4 to be less "
        "aggressive."
    ),
    code(
        "result = await run_guarded(\n"
    ...
    md(
        ...
        "- Step 1: `web_search` called with the **same** args — guard count = 2 ≥ threshold, **trips** — "
        "the canned `\"You've called this tool with these exact arguments multiple times...\"` message "
        "is injected as the tool result instead of running the tool\n"
        "- Step 2: model reads the injected message, gives a final answer, natural exit\n"
        "\n"
        "**If your run shows only 2 trace entries** — the model still gave up before the guard triggered. "
        "If your run shows 3+ identical `web_search` calls — the model is following instructions and the "
        "guard is firing as designed. Both are valid outcomes of the same prompt on a non-deterministic "
        "model.\n"
    ),
    md(
        ...
        "\n"
        "**Never rely on one alone.** A single LLM call can blow `max_tokens` while staying under "
        "`max_steps`. A fast tool can blow `max_cost_usd` on an o1 model while staying under `max_wall_clock`.\n"
    ),
    md(
        "## 8. Self-check\n"
        "\n"
        ...
        "## What's next\n"
        "\n"
        "Notebook **04** — we leave the agent for a moment and build our first **MCP server** with FastMCP. "
        "Five lines of code, one decorator, and the MCP Inspector to debug it. This is the gateway into "
        "S7 territory.\n"
    ),
]

write("notebooks/03_termination.ipynb", cells)
print("wrote notebooks/03_termination.ipynb")
