...
cells = [
    ...
    code(
        ...
        "\n"
        "# **Use snap.config directly** as the fork config. The snapshot already carries\n"
        "# thread_id, checkpoint_id, AND checkpoint_ns — hand-crafting a partial dict\n"
        "# and then handing it to aput_writes() raises KeyError('checkpoint_ns') because\n"
        "# AsyncSqliteSaver writes namespaced rows. Always pass the full snapshot config.\n"
        "fork_config = pre_synth.config\n"
        "\n"
        ...
        "# After aupdate_state, the canonical 'where to resume from' is encoded in the\n"
        "# thread's current state, so we pass the thread-level config (not fork_config\n"
        "# which is pinned to the old checkpoint).\n"
        "thread_config = {\"configurable\": {\"thread_id\": THREAD_ID}}\n"
        "forked = await app.ainvoke(None, config=thread_config)\n"
