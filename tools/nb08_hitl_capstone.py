...
cells = [
    ...
    md(
        ...
        "\n"
        "**Have:**\n"
        "- ReAct loop with parallel tool calls + graceful termination\n"
        "- Strict mode + structured tool layer\n"
        "- Cost / loop / step caps with budget enforcement\n"
        "- Two MCP servers, two transports, generic adapter into the tool registry\n"
        "- Coordinator + parallel researchers via A2A protocol\n"
        "- Tier-2 sync HITL gate with durable approval state\n"
        "\n"
        ...
        "2. **Real observability** — emit OpenTelemetry GenAI spans to Langfuse instead of "
